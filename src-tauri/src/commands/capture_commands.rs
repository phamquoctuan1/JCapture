...
pub fn save_video_recording_bytes(
    ...
    let clean_thumb: String = raw_thumb_b64.chars().filter(|c| !c.is_whitespace()).collect();
    if !clean_thumb.is_empty() {
        if let Ok(thumb_bytes) = base64::engine::general_purpose::STANDARD.decode(&clean_thumb)
            .or_else(|_| base64::engine::general_purpose::URL_SAFE.decode(&clean_thumb))
            .or_else(|_| base64::engine::general_purpose::STANDARD_NO_PAD.decode(&clean_thumb))
        {
...
#[tauri::command]
pub fn export_video_file(file_path: String, default_name: Option<String>) -> Result<Option<String>, String> {
    #[cfg(windows)]
    unsafe {
        use windows::core::PWSTR;
        use windows::Win32::UI::Controls::Dialogs::{GetSaveFileNameW, OPENFILENAMEW, OFN_OVERWRITEPROMPT, OFN_PATHMUSTEXIST};
        ...
        if GetSaveFileNameW(&mut ofn).as_bool() {
            let len = file_buf.iter().position(|&c| c == 0).unwrap_or(file_buf.len());
            let path_str = String::from_utf16_lossy(&file_buf[..len]);
            std::fs::copy(&file_path, &path_str).map_err(|e| e.to_string())?;
            Ok(Some(path_str))
        } else {
            Ok(None)
        }
